...
pub fn get_layout_of(ty: &Type, interner: &TypeInterner) -> Layout {
    match ty {
        ...
        Type::TaglessUnion(variants) => {
            ...
            for v in variants {
                ...
                let layout = get_layout_of(&resolved_v, interner);
...
pub fn get_alignment_of(ty: &Type, interner: &TypeInterner) -> usize {
    get_layout_of(ty, interner).alignment
}
...
fn calculate_fields_layout(field_types: &[Type], interner: &TypeInterner) -> Layout {
    let mut current_offset = 0;
    let mut max_alignment = 1;

    for ty in field_types {
        let field_layout = get_layout_of(ty, interner);
